...
if (inBrowser) {
  try {
    ...
    Object.defineProperty(opts, 'passive', ({
      ...
      /**
       * 监听页面滚动，只有执行了监听器才知道是否设置了preventDefault,即此时才知道是否需要滚动页面，这就会导致滚动延时卡顿，
       * {passive: true}的作用就是告诉浏览器我的监听器无preventDefault,可以直接执行滚动操作
       */
...
if (process.env.NODE_ENV !== 'production') {
  ...
  var classify = function (str) { return str // 转化成首字母大写的名字
...
/**
 * 对数组元素赋值、更改对象属性的值、添加属性的值（如果对象是可观察对象，则设置值为观察者，并发布对象的更改）
...
/**
 * 合并对象，格式化props，inject，directives，优先child上的属性
...
function mergeOptions (
  ...
  if (process.env.NODE_ENV !== 'production') {
    checkComponents(child);// 校验components内的组件名
  }
  ...
  // 格式child的props,inject,directives
...
/**
 * Assert whether a prop is valid. 组件属性检测
 */
function assertProp (
  prop,
  name,
...
function getTypeIndex (type, expectedTypes) {
  ...
  for (var i = 0, len = expectedTypes.length; i < len; i++) { // 属性值可以为多种类型
...
if (process.env.NODE_ENV !== 'production') {
  ...
  var hasHandler = {
    has: function has (target, key) {// 没有该属性且是关键字或者私有属性，则返回false
...
/*  */
// 查找标签上有没有组件选项中的props属性的值，并返回结果（属性：值）
...
/**
 * Runtime helper for rendering <slot>render时解析插槽 _vm._t("default")
 */
function renderSlot (
  name,
  fallback,
...
function _createElement (
  ...
  if (typeof tag === 'string') {
    ...
    vnode = createComponent(tag, data, context, children);// component标签中的is属性时组件选项对象,tag指向组件选项对象,data值为{tag:"component"}
  }
...
function initRender (vm) {
  ...
  var renderContext = parentVnode && parentVnode.context;// 父组件
  vm.$slots = resolveSlots(options._renderChildren, renderContext);// 获取组件插槽内容（Vnode实例）{default: [Vnode实例,...]}
...
function renderMixin (Vue) {
  ...
  installRenderHelpers(Vue.prototype);// 添加方法在渲染模板时调用
...
function initEvents (vm) {
  ...
  var listeners = vm.$options._parentListeners;// 组件标签上的事件{test:function test() { console.log('父组件上的方法') }}
  if (listeners) {
...
function add (event, fn) { // 添加绑定在组件标签上的事件
  target.$on(event, fn);
}

function remove$1 (event, fn) {// 移除绑定在组件标签上的事件
  target.$off(event, fn);
}

function createOnceHandler (event, fn) {// 添加绑定在组件标签上的事件，事件触发之后移除
  var _target = target;
...
function updateComponentListeners (
  ...
  target = vm;
  updateListeners(listeners, oldListeners || {}, add, remove$1, createOnceHandler, vm);// 更新绑定在组件标签上的监听事件
  target = undefined;
}
...
function initLifecycle (vm) {
  ...
  if (parent && !options.abstract) {
    while (parent.$options.abstract && parent.$parent) { // 向上查找非抽象父组件
      parent = parent.$parent;
    }
    parent.$children.push(vm); // 将子组件添加到父组件的$children数组中
  }

  vm.$parent = parent;
  vm.$root = parent ? parent.$root : vm;// 设置根组件实例
...
function lifecycleMixin (Vue) {
  ...
  Vue.prototype.$destroy = function () {
    ...
    vm._isBeingDestroyed = true;// 置为销毁状态
    // remove self from parent
    var parent = vm.$parent;
    if (parent && !parent._isBeingDestroyed && !vm.$options.abstract) {// 如果父节点不销毁且当前节点不是抽象组件，则移除父节点中的当前节点。
...
/** 观察者，发生变化通知观察者
 * A watcher parses an expression, collects dependencies,收集
...
Watcher.prototype.addDep = function addDep (dep) {
  ...
  if (!this.newDepIds.has(id)) {
    ...
    this.newDeps.push(dep);// 将依赖收集器添加到观察者中
    if (!this.depIds.has(id)) {
      dep.addSub(this);// 将观察者添加到依赖收集器中
    }
  }
};
...
/**
 * 包含当前观察者的依赖收集器添加当前激活的观察者
 * Depend on all deps collected by this watcher.
 */
Watcher.prototype.depend = function depend () {
  var i = this.deps.length;
  while (i--) {
    this.deps[i].depend();// 添加
  }
};
...
Watcher.prototype.teardown = function teardown () {
  if (this.active) {
    ...
    if (!this.vm._isBeingDestroyed) { // 如果组件不处于销毁状态，就移除当前观察者
      remove(this.vm._watchers, this);
    }
    var i = this.deps.length;
    while (i--) {
      this.deps[i].removeSub(this);// 依赖收集器中删除当前观察者
    }
    this.active = false;
  }
};
...
var computedWatcherOptions = { lazy: true };// 懒执行，非立即执行
  ...
  return function computedGetter () {
    ...
    if (watcher) {
      if (watcher.dirty) {// true，懒执行
        watcher.evaluate();// 执行watcher方法后设置dirty为false,并把watcher添加到coomputed属性值所依赖的值的依赖收集器中。
      }
      if (Dep.target) {
        watcher.depend();// 把当前激活的watcher(页面更新watcher)添加到当前computed属性值的依赖值的依赖收集器中。
      }
...
//**校验methods方法名，并且绑定this，添加到组件实例上 */
 ...
 */
  Vue.prototype._init = function (options) {
    ...
    vm._isVue = true;
    // merge options
    if (options && options._isComponent) { // 通过extend生成的组件实例_isComponent = true
      // optimize internal component instantiation
      // since dynamic options merging is pretty slow, and none of the
      // internal component options needs special treatment.
      initInternalComponent(vm, options);// 初始化extend生成的组件实例
    } else {
      vm.$options = mergeOptions(// 合并构造器和参数的options，同时格式化实例的props,inject,directives属性（构造器的options是全局属性（混入），影响到每个组件）
...
function initInternalComponent (vm, options) {
  ...
  var parentVnode = options._parentVnode;// 组件节点
  opts.parent = options.parent; // 组件标签所在的组件实例（父组件）
  opts._parentVnode = parentVnode;

  var vnodeComponentOptions = parentVnode.componentOptions;// 组件选项以及组件标签上的信息
  opts.propsData = vnodeComponentOptions.propsData;// 组建标签上的props
  opts._parentListeners = vnodeComponentOptions.listeners;// 组件标签上的事件
  opts._renderChildren = vnodeComponentOptions.children;// 组件标签内容(以VNode实例保存)
  opts._componentTag = vnodeComponentOptions.tag;// 组件名称||组件标签名
...
function resolveConstructorOptions (Ctor) {
  var options = Ctor.options;
  if (Ctor.super) {// Vue.extend生成的实例
    var superOptions = resolveConstructorOptions(Ctor.super);
    var cachedSuperOptions = Ctor.superOptions;// 父类的options
    if (superOptions !== cachedSuperOptions) {
      // super option changed,
      // need to resolve new options.
      Ctor.superOptions = superOptions;// 以最新的父类options为准
      // check if there are any late-modified/attached options (#4976)
      var modifiedOptions = resolveModifiedOptions(Ctor);
      // update base extend options
      if (modifiedOptions) {
        extend(Ctor.extendOptions, modifiedOptions);// 参数options和最新的构造器opions合并
      }
...
function Vue (options) {
  if (process.env.NODE_ENV !== 'production' &&
    !(this instanceof Vue)// 判断this的构造器是不是Vue
  ) {
...
function initExtend (Vue) {
  ...
  Vue.extend = function (extendOptions) {
    extendOptions = extendOptions || {};// 组件选项
    var Super = this;
    var SuperId = Super.cid;// 父级构造器上的cid
    ...
    Sub.cid = cid++;// 构造器id
    Sub.options = mergeOptions(// 合并父级构造器选项和组件选项(全局属性应用到每个组件，全局属性通过构造器传递)
      Super.options,
      extendOptions
    );
    Sub['super'] = Super;
    ...
    if (Sub.options.props) {
      initProps$1(Sub);// 实例上的props属性值由实例的_props属性代理
    }
    ...
    Sub.extend = Super.extend;// 复用父级extend
    Sub.mixin = Super.mixin;// 复用父级mixin
    Sub.use = Super.use;// 复用父级use
...
function initProps$1 (Comp) {
  ...
  for (var key in props) {
    proxy(Comp.prototype, "_props", key);// 实例上的props属性值由实例的_props属性代理
  }
}
...
var KeepAlive = {
  ...
  render: function render () {
    var slot = this.$slots.default;// 获取子标签节点
    ...
    if (componentOptions) {
      ...
      if (
        ...
        // excluded
        (exclude && name && matches(exclude, name))// exclude有name
      ) {
...
function initGlobalAPI (Vue) {
  ...
  Vue.options = Object.create(null);// 将component,directive,filter添加到
  ASSET_TYPES.forEach(function (type) {
  ...
  //将内建组件添加到Vue.options.components上
...
function createPatchFunction (backend) {
  ...
  function createElm (
    ...
    if (createComponent(vnode, insertedVnodeQueue, parentElm, refElm)) {// 如果vnode是组件节点
      return
    }
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        if (isRealElement) {
          ...
          oldVnode = emptyNodeAt(oldVnode);// 根据标签元素创建VNode节点实例
        }
        ...
        // destroy old node
        if (isDef(parentElm)) {
          removeVnodes(parentElm, [oldVnode], 0, 0);// 移除旧节点以及DOM元素
...
function remove$2 (// 移除原生DOM元素上的事件
  name,
  handler,
  capture,
  _target
...
function createCompilerCreator (baseCompile) {
  return function createCompiler (baseOptions) {
    ...
    return {
      compile: compile,
      compileToFunctions: createCompileToFunctionFn(compile) // 将模板解析成render渲染函数
    }
  }
}
...
// module id = ./node_modules/vue/dist/vue.esm.js
// module chunks = 34
