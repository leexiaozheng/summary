...
(function (global, factory) {
  ...
  var View = {
    ...
    render: function render (_, ref) {// ref
      var props = ref.props;
      var children = ref.children;
      var parent = ref.parent;// 标签所处组件的实例
      var data = ref.data;
      ...
      var name = props.name;
      var route = parent.$route;// 路由根组件上的_route,当_route发生变化时触发该抽象组件重新渲染
      ...
      var depth = 0;
      var inactive = false;
      while (parent && parent._routerRoot !== parent) {// 不在路由根组件模板里
        var vnodeData = parent.$vnode ? parent.$vnode.data : {};
        if (vnodeData.routerView) {
          depth++;// 当前组件标签到路由根组件之间有多少个routerView
        }
      ...
      // cache component
      cache[name] = { component: component };// 缓存组件选项
...
// 解析路由参数（query），并合并当前路由参数（extraQuery）
  ...
  function install (Vue) {
    ...
    Vue.mixin({
      beforeCreate: function beforeCreate () {
        if (isDef(this.$options.router)) {
          ...
          Vue.util.defineReactive(this, '_route', this._router.history.current);// 设置路由根组件上的_route属性响应式
        } else {
    ...
    Object.defineProperty(Vue.prototype, '$router', {
      get: function get () { return this._routerRoot._router }// 返回路由根组件上的_route属性
    });

    Object.defineProperty(Vue.prototype, '$route', {// 返回路由根组件上的_route属性
  ...
  function addRouteRecord (
    ...
    var pathToRegexpOptions =
      route.pathToRegexpOptions || {};// 路径正则匹配的参数
    var normalizedPath = normalizePath(path, parent, pathToRegexpOptions.strict);// 格式路径，返回完整路径
    ...
    var record = {
      path: normalizedPath,
      regex: compileRouteRegex(normalizedPath, pathToRegexpOptions),// 正则匹配设置参数
 ...
 */
  function normalizePath (
    ...
    if (!strict) { path = path.replace(/\/$/, ''); }// 正常模式去除字符串末尾的“/”
    if (path[0] === '/') { return path }// 绝对路径直接返回
    if (parent == null) { return path }// 根路径直接返回
    return cleanPath(((parent.path) + "/" + path))// 将“//”转成“/”
  }
 ...
 */
  ...
  function genStateKey () {// 返回唯一值
    return Time.now().toFixed(3)// 保留3为小数
  }
  ...
  function setupScroll () {
    ...
    window.addEventListener('popstate', handlePopState);// 当活动历史记录条目更改时,将触发popstate事件
    return function () {
      window.removeEventListener('popstate', handlePopState);
    }
  }

  function handleScroll (// 页面滚动
    router,
    to,
    from,
    isPop
  ) {
    if (!router.app) {// 有router的Vue根实例
      return
    }

    var behavior = router.options.scrollBehavior;
    if (!behavior) {//没有滚动行为参数
      return
    }
    ...
    router.app.$nextTick(function () {
      var position = getScrollPosition();// 获取之前的滚动位置
      var shouldScroll = behavior.call(
        router,
        to,
        from,
        isPop ? position : null// 是否返回头部
      );
...
// 滚动页面到指定位置
  ...
  function pushState (url, replace) {
    saveScrollPosition();// 根据key保存口左上角水平和垂直方向滚动的像素
    ...
    try {
      if (replace) {
        ...
        stateCopy.key = getStateKey();
        history.replaceState(stateCopy, '', url);// 修改当前的记录（（并不会引起页面加载，只是更改了url或者状态对象）
      } else {
        history.pushState({ key: setStateKey(genStateKey()) }, '', url);// 添加一条记录（并不会引起页面加载，只是更改了url）
      }
    } catch (e) {
  ...
  function normalizeBase (base) {
    if (!base) {
      if (inBrowser) {
        ...
        base = base.replace(/^https?:\/\/[^\/]+/, '');// 去除origin
      } else {
        base = '/';
      }
    }
...
// 返回组件实例的beforeRouteUpdate钩子方法(方法this指向组件实例)
  ...
  var HTML5History = /*@__PURE__*/(function (History) {//继承了History
    ...
    if ( History ) HTML5History.__proto__ = History;// 继承原生history，复用构造器上的方法
    HTML5History.prototype = Object.create( History && History.prototype );// 继承，复用实例上的方法
    ...
    HTML5History.prototype.go = function go (n) {// 路由回退或者前进
      window.history.go(n);
    };

    HTML5History.prototype.push = function push (location, onComplete, onAbort) {// 路由跳转
      var this$1 = this;
 ...
 */
  ...
  function getLocation (base) {// 通过页面的url和基础路径获取路由配置的路径
    var path = decodeURI(window.location.pathname);// 获取路径（定位路径，不包含origin）
    if (base && path.toLowerCase().indexOf(base.toLowerCase()) === 0) {
      path = path.slice(base.length);// 去除基础路径部分
    }
  ...
  var VueRouter = function VueRouter (options) {
    ...
    this.fallback = mode === 'history' && !supportsPushState && options.fallback !== false;//fallback,当浏览器不支持 history.pushState 控制路由是否应该回退到 hash 模式。默认值为 true
    if (this.fallback) {
      mode = 'hash';
    }
    if (!inBrowser) {
    ...
    switch (mode) {// options.base 基路径（路径前缀）
      case 'history':
  ...
  VueRouter.prototype.init = function init (app /* Vue component instance */) {// 每个组件初始化路由
      var this$1 = this;
    ...
    app.$once('hook:destroyed', function () {
      ...
      if (index > -1) { this$1.apps.splice(index, 1); }// 组件销毁时从apps移除
