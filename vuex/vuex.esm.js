...
ModuleCollection.prototype.register = function register (path, rawModule, runtime) {
  ...
  if (path.length === 0) {
    ...
    var parent = this.get(path.slice(0, -1));// 获取父模块
    parent.addChild(path[path.length - 1], newModule);// 将模块映射到父元素的_childreny中，完成父子模块之间的引用
  }

  // register nested modules
  if (rawModule.modules) {// 递归注册子模块
...
function assertRawModule (path, rawModule) {// 判断actions,mutations,getters类型是否正确
...
var Store = function Store (options) {
  var this$1 = this;
  if ( options === void 0 ) options = {};// options初始化
  ...
  this.dispatch = function boundDispatch (type, payload) {// 在store实例上添加dispatch方法
    return dispatch.call(store, type, payload)
  };
  this.commit = function boundCommit (type, payload, options) {// 在store实例上添加commit方法
...
Store.prototype.subscribe = function subscribe (fn) {
  return genericSubscribe(fn, this._subscribers)// 添加订阅mutation事件，并返回函数，调用该函数取消订阅
};

Store.prototype.subscribeAction = function subscribeAction (fn) {
  var subs = typeof fn === 'function' ? { before: fn } : fn;
  return genericSubscribe(subs, this._actionSubscribers)// 添加订阅action事件，并返回函数，调用该函数取消订阅
};

Store.prototype.watch = function watch (getter, cb, options) {// getter函数读取state或者getters值，当值发生变化时调用cb函数
    var this$1 = this;
  ...
  Object.defineProperties(local, {
    getters: {
      get: noNamespace
        ...
        : function () { return makeLocalGetters(store, namespace); }// 读取值时调用，将之前以命名空间+name保存在store的getter以命名空间:{name:}形式保存在store上
    },
    state: {
...
function registerMutation (store, type, handler, local) {
  ...
  entry.push(function wrappedMutationHandler (payload) {// 绑定其他值，调用时只需要传一个参数：值
...
// 根据路径获取模块的state(子模块的state是存放在父模块的state中)
...
function normalizeNamespace (fn) {
  return function (namespace, map) {
    if (typeof namespace !== 'string') {// 非模块state，无命名空间
      map = namespace;
      namespace = '';
